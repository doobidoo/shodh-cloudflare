...
function parseTemporalExpression(expr: string): string | null {
  ...
  match = lowerSince.match(/^(\d+)\s+days?\s+ago$/i) || lowerSince.match(/^vor\s+(\d+)\s+tagen$/i);
  if (match) {
    const daysAgo = parseInt(match[1]);
    return getStartOfDay(new Date(now.getTime() - daysAgo * 24 * 60 * 60 * 1000)).toISOString();
  }
  ...
  match = lowerSince.match(/^(\d+)\s+weeks?\s+ago$/i) || lowerSince.match(/^vor\s+(\d+)\s+wochen$/i);
  if (match) {
    const weeksAgo = parseInt(match[1]);
    const daysAgo = weeksAgo * 7;
    return getStartOfDay(new Date(now.getTime() - daysAgo * 24 * 60 * 60 * 1000)).toISOString();
  ...
  match = lowerSince.match(/^(\d+)\s+months?\s+ago$/i) || lowerSince.match(/^vor\s+(\d+)\s+monaten$/i);
  if (match) {
    const monthsAgo = parseInt(match[1]);
    const target = new Date(now.getFullYear(), now.getMonth() - monthsAgo, 1);
    return getFirstOfMonth(target).toISOString();
